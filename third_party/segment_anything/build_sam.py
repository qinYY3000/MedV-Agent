...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        state_dict = torch.load(open(checkpoint, "rb"), map_location=map_device, weights_only=True)
        # 修复 key 前缀: text_model.text_model.xxx -> text_model.xxx
        new_state = {}
        for k, v in state_dict.items():
            new_k = k.replace("text_model.text_model.", "text_model.")
            new_state[new_k] = v
        missing, unexpected = sam.load_state_dict(new_state, strict=False)
        if missing:
            print(f'Missing keys: {len(missing)} (text_model keys expected for IMISNet)')
        if unexpected:
            print(f'Unexpected keys: {len(unexpected)}')
        print(f'******Loaded IMISNet parameters ({map_device})')

...
class AsyncRolloutRequest(BaseModel):
    ...
    def _get_position_ids(
        ...
        # special case for qwen2vl and qwen3vl
        is_qwen_vl = (
            hasattr(processing_class, "image_processor")
            and "Qwen2VLImageProcessor" in processing_class.image_processor.__class__.__name__
        )
        if is_qwen_vl:
            # Check if it's Qwen3VL or Qwen2.5VL based on processor class name
            is_qwen3vl = "Qwen3VLProcessor" in processing_class.__class__.__name__
            if is_qwen3vl:
            ...
            new_position_ids = torch.cat((text_position_ids, vision_position_ids), dim=0)
            return new_position_ids  # (4, seq_len)
        else:

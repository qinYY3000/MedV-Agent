...
class SGLangRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        """
        logger.warning(f"SGLANG_ROLLOUT_CALLED: multi_turn.enable={self.config.multi_turn.enable}")
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if position_ids.dim() == 3:  # qwen2vl/qwen3vl mrope (batch size, 4, seq len)
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if prompt_position_ids[0].dim() == 2:
            ...
            # e.g. from qwen2vl/qwen3vl, prompt_position_ids.shape = (4, seq_len) where 4 = text + 3D vision
        ...
        if response_position_ids[0].dim() == 2:
            ...
            # e.g. from qwen2vl/qwen3vl, response_position_ids.shape = (4, seq_len) where 4 = text + 3D vision
        ...
        is_multimodal = isinstance(self.processing_class, ProcessorMixin) and (
            hasattr(self.processing_class, "image_processor") or hasattr(self.model_config.hf_config, "vision_config")
        )
    ...
    def _create_padding_request(self, original_req: AsyncRolloutRequest) -> AsyncRolloutRequest:
        ...
        if original_req.position_ids is not None:
            first_dim = 1
            # if position_ids is a 2D tensor (e.g. qwen2vl/qwen3vl)

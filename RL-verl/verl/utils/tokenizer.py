...
def hf_tokenizer(name_or_path, correct_pad_token=True, correct_gemma2=True, **kwargs):
    ...
    # Fix: Qwen3-VL tokenizer config has list-type extra_special_tokens,
    # which breaks transformers < 5.0. Convert to dict format.
    if "extra_special_tokens" not in kwargs:
        config_path = os.path.join(name_or_path, "tokenizer_config.json")
        if os.path.exists(config_path):
            try:
                ...
                if isinstance(extra, list):
                    kwargs["extra_special_tokens"] = {tok: tok for tok in extra}
            except Exception:
                pass

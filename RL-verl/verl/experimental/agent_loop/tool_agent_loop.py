...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        # Extract step and index for logging
        ...
        # NOTE: For MedSAM, agent_data.image_data contains PIL Images (original + overlays), NOT masks
        # The actual predicted mask should be stored in extra_fields by the subclass
        # (e.g., MedSAMIterativeAgentLoop stores it as extra_fields["pred_mask"])
        # So we don't extract final_mask from image_data here
        ...
        output.extra_fields.update({
            ...
            # NOTE: pred_mask should be set by subclass (e.g., MedSAMIterativeAgentLoop)
            # because it knows how to extract the actual mask from tool responses
            # NOTE: num_turns is already set in output.num_turns, no need to duplicate in extra_fields
        })
    ...
    async def _handle_generating_state(
        ...
        # Always add assistant response to messages for conversation logging
    ...
    async def _call_tool(
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Problematic arguments: {tool_call.arguments[:200]}")
        ...
        # Handle errors - return early if tool execution failed
        if not success:
            return (
                ToolResponse(
                    text=f"Error when executing tool: {error_msg}",
                ),
                0.0,
                {},
            )

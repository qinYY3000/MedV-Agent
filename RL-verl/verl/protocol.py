...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        for key, val in non_tensor_batch.items():
            # Debug: log shapes/dtypes/ndims before concatenate
            shapes_info = []
            for i, arr in enumerate(val):
                try:
                    ...
                    shapes_info.append({'index': i, 'error': str(e), 'type': type(arr).__name__})
            
            # Log detailed shape info for debugging dimension mismatches
            logging.info(f"[DataProto.concat] key='{key}', shapes_info={shapes_info}")

            # Check if all arrays have the same ndim and shape (except axis 0)
            ndims = [info['ndim'] for info in shapes_info if info.get('ndim') is not None]
            shapes = [info['shape'] for info in shapes_info if info.get('shape') is not None]
            
            # Check for ndim mismatch OR shape mismatch (for ndim >= 2)
            needs_normalization = False
            if ndims and len(set(ndims)) > 1:
                needs_normalization = True
            elif ndims and ndims[0] >= 2:
                # Check if shapes match along all axes except axis 0
                reference_shape = shapes[0][1:]  # Shape excluding first dimension
                for i, shape in enumerate(shapes):
                    if shape[1:] != reference_shape:
                        needs_normalization = True
                        break
            
            if needs_normalization:
                # Normalize to 1D object arrays
                normalized_val = []
                for i, arr in enumerate(val):
                    if isinstance(arr, np.ndarray):
                        if arr.ndim == 1:
                            # Already 1D, keep as-is
                            normalized_val.append(arr)
                        else:
                            # Convert multi-dimensional array to 1D object array
                            # Each element becomes a row from the original array
                            logging.info(f"[DataProto.concat] Converting worker {i}'s {arr.shape} array to 1D object array")
                            obj_arr = np.empty(arr.shape[0], dtype=object)
                            for j in range(arr.shape[0]):
                                obj_arr[j] = arr[j]
                            normalized_val.append(obj_arr)
                    else:
                        normalized_val.append(arr)
                val = normalized_val
            ...
            try:
                ...
                logging.error(f"[DataProto.concat] Failed to concatenate key='{key}'")
                logging.error(f"  Error: {e}")
                logging.error(f"  Number of arrays: {len(val)}")
                for i, arr in enumerate(val):
                    if hasattr(arr, 'shape'):
                        logging.error(f"    Array {i}: shape={arr.shape}, dtype={arr.dtype}")
                    else:
                        logging.error(f"    Array {i}: type={type(arr)}, value={arr}")
                raise
